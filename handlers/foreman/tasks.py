...
from db.repo import (
    ...
    get_task_by_id, get_site_by_id, get_site_worker_telegram_ids, get_worker_lang
)
...
async def foreman_task_detail(callback: CallbackQuery, session: AsyncSession, lang: str):
    task_id = int(callback.data.split(":")[1])
    task = await get_task_by_id(session, task_id)
    if not task:
        await callback.answer(t("task_not_found", lang), show_alert=True)
    ...
    await callback.message.edit_text(
        ...
        reply_markup=kb_task_foreman(task.id, lang),
    )
    await callback.answer()
...
async def foreman_task_desc(message: Message, state: FSMContext, session: AsyncSession, current_user, bot: Bot, lang: str):
    ...
    worker_tg_ids_with_ids = await get_site_worker_telegram_ids(session, data["site_id"])
    site = await get_site_by_id(session, data["site_id"])
    for tg_id in worker_tg_ids_with_ids:
        w_lang = await get_worker_lang_by_tg(session, tg_id)
        try:
            await bot.send_message(
                tg_id,
                t("new_task_notify", w_lang, site=site.name if site else "", title=task.title, desc=task.description)
            )
        except Exception:
            pass
